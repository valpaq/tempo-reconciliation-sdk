...
export type MemoV1 = {
  v: 1;
  /** Memo type. Short name to match the 1-byte on-chain field; use `type` when encoding. */
  t: MemoType;
  issuerTag: bigint;
  ulid: string;
  id16: Uint8Array;
...
export type WatchWsOptions = {
  ...
  dedupeTtlMs?: number;
  onError?: (err: Error) => void;
  /** Max consecutive reconnection attempts without a successful event. Resets to 0 on any successful log batch. 0 = no reconnect. Default: 5. */
...
/**
 * Persistence backend for the Reconciler.
 * All methods must be synchronous — the Reconciler calls them without await.
 */
...
export type ReconcilerOptions = {
   ...
   * - `"each"`: tolerance applies per-payment. A single underpayment beyond tolerance
   *   is `mismatch_amount`. There is no accumulation — `partial` status is never emitted
   *   in this mode. Each payment must independently match within tolerance.
   */
