...
/** Client for the Tempo Explorer API (`https://explore.tempo.xyz/api`). */
export class ExplorerClient {
   ...
   */
  getMetadata(address: `0x${string}`): Promise<AddressMetadata> {
   ...
   * @throws `Error` with `"Explorer API <status>"` on non-2xx response
   */
  getBalances(address: `0x${string}`): Promise<BalancesResponse> {
  ...
  private async fetchJson<T>(path: string): Promise<T> {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Explorer API ${res.status}: ${path}${body ? ` — ${body}` : ""}`);
    }
