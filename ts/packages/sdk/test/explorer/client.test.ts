...
describe("createExplorerClient (error handling)", () => {
  ...
  it("error message includes response body when present", async () => {
    fetchSpy.mockResolvedValueOnce(new Response("account not found", { status: 404 }));
    const client = createExplorerClient();
    await expect(client.getMetadata("0xbeef")).rejects.toThrow("account not found");
  });

  it("error message omits body separator when body is empty", async () => {
    fetchSpy.mockResolvedValueOnce(new Response("", { status: 404 }));
    const client = createExplorerClient();
    const err = await client.getMetadata("0xbeef").catch((e: Error) => e);
