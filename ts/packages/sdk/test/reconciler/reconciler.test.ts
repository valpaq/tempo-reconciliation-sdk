...
describe("Reconciler constructor", () => {
  it("throws on amountToleranceBps > 10000", () => {
    expect(() => new Reconciler({ amountToleranceBps: 10001 })).toThrow(
      "amountToleranceBps must be an integer between 0 and 10000",
    );
  });

  it("throws on negative amountToleranceBps", () => {
    expect(() => new Reconciler({ amountToleranceBps: -1 })).toThrow(
      "amountToleranceBps must be an integer between 0 and 10000",
    );
  });

  it("throws on fractional amountToleranceBps", () => {
    expect(() => new Reconciler({ amountToleranceBps: 1.5 })).toThrow(
      "amountToleranceBps must be an integer between 0 and 10000",
    );
  });

  it("accepts amountToleranceBps at boundary values", () => {
    expect(() => new Reconciler({ amountToleranceBps: 0 })).not.toThrow();
    expect(() => new Reconciler({ amountToleranceBps: 10000 })).not.toThrow();
  });
});
...
describe("Reconciler", () => {
  ...
  it("removeExpected decrements totalExpected and totalExpectedAmount in report", () => {
    const r = new Reconciler();
    ...
    r.removeExpected(memo2);

    const report = r.report();
    expect(report.summary.totalExpected).toBe(1);
    expect(report.summary.totalExpectedAmount).toBe(10_000_000n);
  ...
  it("reset() zeroes totalExpected and totalExpectedAmount in report", () => {
    const r = new Reconciler();
    const memo1 = makeMemo("01MASW9NF6YW40J40H289H858P");
    const memo2 = makeMemo("01MASW9NF6YW40J40H289H8580");
    r.expect(makeExpected({ memoRaw: memo1, amount: 10_000_000n }));
    ...
    expect(report.summary.totalExpectedAmount).toBe(0n);
  });
...
describe("Reconciler: partialToleranceMode", () => {
  ...
  it("each mode: two 50% payments both get mismatch_amount (no accumulation)", () => {
    const memo = makeMemo();
    const r = new Reconciler({
      allowPartial: true,
      partialToleranceMode: "each",
    ...
    const r1 = r.ingest(makeEvent({ memoRaw: memo, amount: 5_000_000n, logIndex: 0 }));
    expect(r1.status).toBe("mismatch_amount");
    ...
    expect(r2.status).toBe("mismatch_amount");
  });
...
describe("Reconciler: allowOverpayment=false + allowPartial", () => {
  it("rejects single overpayment when allowOverpayment=false", () => {
    const r = new Reconciler({ allowOverpayment: false });
    r.expect(makeExpected({ amount: 10_000_000n }));
    const result = r.ingest(makeEvent({ amount: 12_000_000n }));
    expect(result.status).toBe("mismatch_amount");
    expect(result.reason).toMatch(/overpaid/);
  });

  it("final mode: cumulative overpay is rejected as mismatch_amount when allowOverpayment=false", () => {
    const memo = makeMemo();
    const r = new Reconciler({
      allowPartial: true,
    ...
    r.ingest(makeEvent({ memoRaw: memo, amount: 6_000_000n }));
    const r2 = r.ingest(
      makeEvent({
        memoRaw: memo,
        amount: 6_000_000n,
    ...
    // Cumulative = 12M > 10M — allowOverpayment=false rejects the overpayment
    expect(r2.status).toBe("mismatch_amount");
    expect(r2.overpaidBy).toBe(2_000_000n);
    expect(r2.reason).toMatch(/overpaid/);
  });

  it("final mode: cumulative exactly matches amount when allowOverpayment=false", () => {
    const memo = makeMemo();
    const r = new Reconciler({
      allowPartial: true,
      allowOverpayment: false,
    ...
    // Cumulative = 10M = 10M — exact match, no overpayment
    expect(r2.status).toBe("matched");
    expect(r2.overpaidBy).toBeUndefined();
  });

  it("each mode: single payment within tolerance matches even with allowOverpayment=false", () => {
    const r = new Reconciler({
      allowPartial: true,
      allowOverpayment: false,
      amountToleranceBps: 100,
    ...
    r.expect(makeExpected({ amount: 10_000_000n }));
    // 9.95M — underpaid by 50K, within 1% tolerance (100K)
    const result = r.ingest(makeEvent({ amount: 9_950_000n }));
    expect(result.status).toBe("matched");
    expect(result.overpaidBy).toBeUndefined();
  });
...
describe("Reconciler: removeExpected + ingest", () => {
  it("ingest after removeExpected returns unknown_memo", () => {
    const memo = makeMemo();
    const r = new Reconciler();
    r.expect(makeExpected({ memoRaw: memo }));
    r.removeExpected(memo);
    const result = r.ingest(makeEvent({ memoRaw: memo }));
    expect(result.status).toBe("unknown_memo");
  });
});

describe("Reconciler: each mode mismatch then correct", () => {
  it("each mode: mismatch_amount then correct payment matches", () => {
    const memo = makeMemo();
    const r = new Reconciler({
      allowPartial: true,
      partialToleranceMode: "each",
    ...
    const r1 = r.ingest(makeEvent({ memoRaw: memo, amount: 5_000_000n }));
    expect(r1.status).toBe("mismatch_amount");
...
describe("Reconciler edge cases", () => {
  it("ingestMany([]) returns empty array", () => {
    const r = new Reconciler();
    expect(r.ingestMany([])).toEqual([]);
  });

  it("strictSender=true with expected.from undefined matches any sender", () => {
    const r = new Reconciler({ strictSender: true });
    // No 'from' on the expected payment
    r.expect(makeExpected({ from: undefined }));
    const result = r.ingest(makeEvent({ from: "0x9999999999999999999999999999999999999999" }));
    expect(result.status).toBe("matched");
  });
});
