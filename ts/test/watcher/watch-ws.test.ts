...
describe("watchTip20TransfersWs", () => {
  ...
  it("reconnects on WebSocket error with exponential backoff", () => {
    const onError = vi.fn();
    watchTip20TransfersWs(
      { wsUrl: WS_URL, chainId: 42431, token: TOKEN, onError, maxReconnects: 3, reconnectDelayMs: 100 },
      vi.fn(),
    );
  ...
  it("stops reconnecting after maxReconnects is exhausted", () => {
    const onError = vi.fn();
    watchTip20TransfersWs(
      { wsUrl: WS_URL, chainId: 42431, token: TOKEN, onError, maxReconnects: 2, reconnectDelayMs: 100 },
      vi.fn(),
    );
  ...
  it("stops reconnecting when unsubscribed", () => {
    ...
    const unsub = watchTip20TransfersWs(
      { wsUrl: WS_URL, chainId: 42431, token: TOKEN, onError, maxReconnects: 5, reconnectDelayMs: 100 },
      vi.fn(),
    );
