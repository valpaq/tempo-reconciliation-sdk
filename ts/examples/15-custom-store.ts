...
import { Reconciler, encodeMemoV1, issuerTagFromNamespace } from "../src/index";
import type { ReconcileStore } from "../src/types";
...
class JsonFileStore implements ReconcileStore {
  ...
  removePartial(memoRaw: `0x${string}`): void {
    delete this.data.partials[memoRaw];
    this.save();
  }
...
function main() {
  ...
  console.log(`Loaded from file: ${pending.length} expected, ${results.length} results`);
  console.log(
